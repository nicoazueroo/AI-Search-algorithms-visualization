...
/*PSVGL BEGIN================================================================
Prolog Scalable Vector Graphics Library (PSVGL) by Kamen Kanev
This is a collection of Prolog predicates for drawing lines and text in SVG.
Resulting SVG elements are embedded in HTML for vewing in a www browser.  
----------------------------------------------------------------------------*/
/*-----------
PSVGL helpers
-------------*/
...
% Ensure we dont duplicate edges in reverse
assert_unique_edge(X1, Y1, X2, Y2) :-
...
ids_show(Start, Goal, Path) :-
    ...
    /* SAMPLE QUERIES (COPY PASTE THEM INTO PROLOG)

    (When hit enter the console will write down the path, just press enter again to generate the .html succesfully)
    
RECTANGELE 

generate_rec_grid(5, 5, 1), dfs_show((0, 0), (4, 4), Path).
generate_rec_grid(5, 5, 1), dls_show((0, 0), (4, 4), 20, Path).
generate_rec_grid(5, 5, 1), ids_show((0, 0), (4, 4), Path).



TRIANGLE 

generate_tri_grid(4, 4, 1.5), dfs_show((0, 0), (4.5, 5.196), Path).
generate_tri_grid(4, 4, 1.5), dls_show((0, 0), (4.5, 5.196), 20, Path).
generate_tri_grid(4, 4, 1.5), ids_show((0, 0), (4.5, 5.196), Path).

HEXAGONAL

generate_hex_grid(3, 3, 1.5), dfs_show((3.0, 0.0), (6.0, 2.598076211353316), Path).
generate_hex_grid(5, 5, 1.5), dls_show((3.0, 0.0), (6.0, 2.598076211353316), 8, Path).
generate_hex_grid(5, 5, 1.5), ids_show((3.0, 0.0), (6.0, 2.598076211353316), Path).

PYTHAGOREAN

generate_pyt_grid(3, 3, 1), dfs_show((4,2), (4,3), Path).
generate_pyt_grid(3, 3, 1), dls_show((0,0), (4,4), 10, Path).
generate_pyt_grid(3, 3, 1), ids_show((0,0), (4,4), Path).
...
dfs_show((0,0), (4,4), Path),
get_explored_count(N).

reset_explored_count,
generate_rec_grid(5, 5, 1),
dls_show((0,0), (4,4), 20, Path),
get_explored_count(N).
...
dfs_show((0,0), (4.5,5.196), Path),
get_explored_count(N).


reset_explored_count,
generate_tri_grid(4, 4, 1.5),
ids_show((0,0), (4.5,5.196), Path),
get_explored_count(N).
...
dfs_show((4,2), (4,3), Path),
get_explored_count(N).

reset_explored_count,
generate_pyt_grid(3, 3, 1),
dls_show((0,0), (4,4), 10, Path),
get_explored_count(N).
